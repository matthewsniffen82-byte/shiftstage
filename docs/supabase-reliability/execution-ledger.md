...
| 9 | Transactions/atomicity | Controlled pass released and verified; named later work retained | Closing commit `91d03c844192203a9c787f4701914b9bd01c7d23`; exact Vercel success freshly reconfirmed September 12. Detailed validation and handoffs remain below. |
| 10 | Indexes/query performance | Controlled pass released and verified | Closing commit `58e6a5c04ffa6f3038ee7586aa6317dc87128181`; exact Vercel success freshly reconfirmed September 12. Detailed SQL/preservation evidence remains below. |
...
Realtime audit `1ef18d7502a2904e6e4ae5653e6916e4fde84737` was pushed with matching main references and successful [exact Vercel deployment](https://vercel.com/ai-movie-jobs/shiftstage/GXej8ePTCr2FCM43XNFmThTLsmrG). Health, unauthorized media rejection and thirty readiness checks passed at 15:16:42–49 UTC. Read-only comparisons preserved publication settings, all source/history/marker rows, venues, 452 storage objects, buckets, policies, functions and 113 ledger entries. Step 12 is closed; later HTTP failure tests remain assigned to their planned steps.

## Step 13 payout retry ownership

The inspected plan is `step-13-privileged-operations-audit.md`. Scope the existing service-only payout RPC's unique-key fallback to the requesting dancer, preserving every other financial decision and existing record. A cross-dancer collision must fail without returning the other dancer's receipt. The production function fingerprint was freshly verified before implementation.

All 36 native cases pass, including the historical disclosure reproduction, seven cases that fail against the old function, valid same-owner retries and injected rollback at every financial write. Guarded deployment rehearsals pass repeat rejection, function/permission/RLS drift rejection and transactional rollback of injected data/access changes. No hosted payout or provider transfer is used as a test. Full validation, committed-source deployment, exact commit/push/Vercel and preservation/health gates remain required.

The existing public finance route already namespaces keys with the authenticated user's UUID. This correction adds the missing ownership guarantee inside the privileged RPC; it does not establish a currently exploitable public route. The first full suite caught the migration guard's required `SET search_path =` spelling. After correcting that equivalent SQL syntax, all 5,906 tests passed, along with lint, production build, standalone TypeScript, migration guard and thirty readiness checks. The final guarded-source rehearsal also passed. No production payout or record was mutated during testing. Commit/push, exact source application, Vercel and read-only preservation/health verification remain required.

Payout ownership correction `aa1e34671d5ff3b00574cf91fecc3cf11a85c8f4` was pushed with matching main references and successful [exact Vercel deployment](https://vercel.com/ai-movie-jobs/shiftstage/1Hi1te9aeZsTQrBwCmxMtctCzp7P). Exact migration `20260910153000` applied transactionally; verification preserved all financial records, schema/access, 129 other functions and 113 preceding ledger entries. Independent read-only checks at 15:51:17 and 15:52:59 UTC preserved source/media rows and 452 storage objects, and confirmed 114 ledger entries. Health, anonymous finance/admin/media rejection, unsigned webhook rejection and thirty readiness checks passed at 15:52:57–15:53:03 UTC. No production payout was invoked.

## Step 13 paid payout recovery foundation

The inspected plan is `step-13-paid-payout-recovery.md`. Add a service-only transaction for paid recovery flags and their audit, preserving review reasons and paid balances. First deploy and verify the unused function; only then switch the signed webhook caller in a separate correction.

All 49 focused cases pass, including 46 new native cases. They cover multi-item payouts, cross-reference/state/amount failures, old flags without an audit, preserved review reasons, interrupted writes, safe retries and denied roles. Guarded deployment rehearsals pass repeat rejection, dependency/privacy/RLS drift rejection and transactional preservation rollback. Read-only readiness and migration guards pass. Full validation and exact committed-source delivery/health/preservation gates remain required; the caller is unchanged.

Full validation passed all 5,952 tests, lint, production build, standalone TypeScript, migration guard and thirty readiness checks. The additive function is not yet called by production application code. No production reversal/recovery or financial mutation was tested. Exact commit/push, committed-source application, Vercel and independent preservation/health gates remain required before the caller correction.

Paid-recovery foundation `f61aa2bbbede15453af95fb360e54bbd0d636057` was pushed with matching main references and successful [exact Vercel deployment](https://vercel.com/ai-movie-jobs/shiftstage/CtypgEDeSG98j6RsGcP8jucxb5Sm). Exact migration `20260910160000` applied transactionally, preserving financial rows/schema/access, all 130 existing functions and 114 prior ledger entries. Independent checks at 16:08:01 and 16:10:11 UTC confirmed the new invoker's service-only access, 115 ledger entries and unchanged source/media records and all 452 storage objects. Health, unauthorized finance/admin/media rejection, unsigned webhook rejection and thirty readiness checks passed at 16:10:07–13 UTC. The caller remained unchanged and no recovery RPC was invoked against production.

## Step 13 signed paid-recovery caller

The inspected plan is `step-13-paid-recovery-caller.md`. Replace the paid/partial reversal handler's two writes with the deployed transaction and require a complete same-payout, same-reference receipt. Retain existing transfer-reference checks, early callbacks, partial completion and safe redelivery without an automatic debit or repeated write in one delivery.

All 171 focused cases pass, including 39 new signed-webhook cases that fail against the preceding implementation. They cover malformed receipts, lost acknowledgments after commit, rollback on audit failure, preserved manual review reasons and stale references/states. Read-only checks confirm the prerequisite fingerprint and unchanged financial records. Full validation, exact commit/push/Vercel and preservation/health gates remain required. No schema change or production reversal test is part of this caller correction.

An older source guard was updated to verify the no-debit marker in its new SQL transaction and the caller's RPC. After that test correction, all 5,991 tests passed, together with lint, production build, standalone TypeScript, migration guard and thirty readiness checks. The existing completion/release paths and accounting protections remain covered. No production recovery operation was invoked. Exact commit/push, Vercel and preservation/health gates remain required.

Paid-recovery caller `6dd4cd5c092fdb55e25c4a2e282df292562a6ffd` was pushed with matching main references and successful [exact Vercel deployment](https://vercel.com/ai-movie-jobs/shiftstage/8pHPb55ymDrTh1gGBohHjBtkZSxY). Health, unauthorized finance/admin/media rejection, unsigned webhook rejection and thirty readiness checks passed at 16:28:33–39 UTC. Read-only verification preserved financial rows/schema/access, source/media records, 452 storage objects, 131 functions and 115 ledger entries. No production recovery RPC or business mutation was invoked.

## Step 13 cashier allocation snapshot

The inspected plan is `step-13-cashier-allocation-snapshot.md`. Capture the cashier transaction's agent total, recipients and attribution in one statement snapshot, then insert the captured allocations. Preserve rates, eligibility, locks, security and all existing records. A controlled local interleaving reproduced a 2,400-cent revenue total with only 900 cents of recipient rows against the old definition; read-only production aggregation found no existing mismatch.

All 39 focused database cases pass, including ten that fail against the old function. The actual sixteen target tables retain their columns, constraints, indexes and triggers; supporting tables are documented projections. Full validation and exact committed-source delivery, preservation and health gates remain required. No hosted redemption, payout, agent change or financial mutation was used as a test.
...
Cashier allocation correction `08927cdf2648cd626005e3fa43e6ba6837e41f8c` was pushed with matching main references and successful [exact Vercel deployment](https://vercel.com/ai-movie-jobs/shiftstage/7KoMHQAtYZDstEdfmMitwkHfGAkt). Exact migration `20260910165000` applied transactionally, preserving the sixteen target tables, their schema/access, 130 other functions and 115 preceding ledger entries. Independent verification at 17:00:20–23 UTC confirmed zero allocation mismatches, unchanged financial/source/media rows, 452 storage objects, 131 functions and 116 ledger entries. Health, unauthorized finance/admin/media rejection, unsigned webhook rejection and thirty readiness checks passed at 17:00:17–26 UTC. No production redemption or financial mutation was invoked.

## Step 13 dancer enrollment lock order

The inspected plan is `step-13-dancer-tap-lock-order.md`. Serialize fresh registration and deferred dashboard completion with one transaction lock per dancer before either path locks tag/enrollment rows. The existing order admits a wait cycle; this is a potential concurrency failure, not evidence of a production deadlock or partial commit. Preserve all downstream approval, affiliation and Working Now rules and existing records.

All 25 native cases pass, including ten that fail against the old definitions. They verify held lock identity, per-dancer separation, commit/rollback release, ordering before row access, active and incomplete paths, expiry, oldest pending selection, no deferred Working Now, repeat-tap behavior, downstream rollback, service-only access and preserved metadata. The current eleven-table fixture includes the twelve actual triggers; peripheral gallery tables are explicit projections. Full validation, committed-source delivery and production health/preservation remain required. Independent hosted-session concurrency is still deferred.

After integrating the separately delivered webhook review at `060b2976d3e7856efb2cc0c9ce3c64c0732fb919`, all 6,068 tests passed with zero failures or skips, together with lint, production build, standalone TypeScript, migration guard and thirty read-only readiness checks. Fresh production schema matches the native fixture. The guarded source has SHA-256 `6001c96c8a7caaa5a828e2581f9ab6a79716d67559298ea7d73ce8e8fc8fa031` and MD5 `5c4fe70307f6dcf3027be2a513881400`. Exact commit/push, committed-source application, Vercel and independent preservation/health gates remain required. No production tap, activation, notification or schedule mutation was used as a test.

The first push was safely rejected after concurrent security release `fa3b140543c4c5092be431f43b1c3a37793123d2`. The unpublished correction was rebased, preserving both changes. Final combined validation passed all 6,104 tests, lint, production build, standalone TypeScript, migration check and thirty read-only readiness checks; the committed-source migration rehearsal also passed. Exact push, source application, Vercel and preservation/health remain required.

Dancer enrollment correction `2adc54ba92b64abfb504315f62084b0fbfd852f6` was pushed with matching main references and successful [exact Vercel deployment](https://vercel.com/ai-movie-jobs/shiftstage/FievemMRf2zF7mVqpyUfkHwKry6o). Exact migration `20260910172000` applied transactionally, preserving the eleven target tables, schema/access, 129 other functions and 116 preceding ledger entries. Independent verification at 19:16:06 UTC confirmed unchanged target/source/media records and all 452 storage objects, with 131 functions and 117 ledger entries. Health, anonymous dashboard/finance/admin/media rejection, unsigned webhook rejection and thirty readiness checks passed at 19:17:22–28 UTC. No production tap, activation or dashboard finalization was used as a test. The remaining Step 13 function/caller review continues.

## Step 13 function security release check

The inspected plan is `step-13-function-security-check.md`. Add a read-only catalog gate for effective function access, reviewed browser interfaces, inherited ownership, role capabilities and pinned search paths. Preserve all production definitions, grants and records. The catalog has 131 functions, including 99 definers, four intentional anonymous interfaces and seven authenticated interfaces. The continuing review records the separate sticker-capacity, retired-claim lock-order and DMCA lifecycle handoffs rather than modifying them in this correction.

All 27 native checker cases pass, including intentional privilege/path/definition faults and read-only failure preservation. The fixture retains the seven actual browser definitions with projected supporting types; application functions are not invoked by the checker. The live metadata-only check passed at 19:25:02 UTC. Full validation, exact commit/push/Vercel and independent preservation/health remain required; no schema migration or production mutation is part of this correction.

The checker now explicitly places temporary schemas last, and its 28th native case proves a temporary catalog table cannot conceal an unexpected grant. Four current role/admin definitions match the existing native authorization fixture. After integrating finance error-handling release `fcd8e58e53b5565abe3c6a19c190ff0028987b79`, the final combined suite passed all 6,166 tests, lint, production build, standalone TypeScript, migration guard and thirty readiness checks. The final SQL also passed against production without invoking application functions. Exact commit/push/Vercel and preservation/health gates remain required.

## Step 13 release reconciliation and caller closure

Function-check release `db9dbf0bd908d68843ce9f389c576a2ed18c02b6` has a freshly confirmed successful [exact Vercel deployment](https://vercel.com/ai-movie-jobs/shiftstage/76VDnskiJEviDTd5uYFHb5xmVHAt). On September 12, both health routes returned 200/ok at 04:15:34–35 UTC, thirty readiness checks passed, and the function gate passed at 04:15:48 UTC. Independent review reconfirmed the gate at 04:17:06 UTC and the exact installed dancer-tap functions and migration. A fresh 131-function/117-migration capture matches all 31 recent committed SQL hashes. No SQL was reapplied. Old private row-fingerprint captures were not copied to this checkout, so no retrospective whole-record comparison is claimed; the new release has its own read-only preservation baseline.

The closing plan is `step-13-caller-review.md`, with the 131-function metadata/caller map and explicit later-step handoffs. Twenty-six new actual-route tests pass for verified administrator identity, single invocation after uncertain outcomes, and retained retirement boundaries. The existing administrator/claim suite passes all 364 cases. A synthetic actor-substitution mutation is detected.

Release reconciliation found seventeen applied migrations still absent from the history guard. Their exact committed SQL matches one live ledger entry each; eighteen latest function bodies and thirteen schema checks match the current deployed effects. Extend the manifest only with these verified hashes, preserving all existing 142 entries and metadata. Eighteen tests prove that edits to every added migration were previously admitted and are now rejected. The remote ledger and SQL files remain unchanged; historical replay remains deferred and `replayReady` remains false.

The combined change adds 44 passing focused tests and audit records plus the verified manifest entries. Full suite, lint, production build, standalone TypeScript, exact commit/push/Vercel and read-only post-deployment preservation/health are required before Step 14 begins. Concurrent demo, security, review and architecture releases are preserved; this task applies no SQL or production business mutation.
...
## Step 13 closing release delivery

Closing commit `f1cb4ff3b554c431b1195facdde7b7a2c3f218d5` was pushed with matching clean main references and successful [exact Vercel deployment](https://vercel.com/ai-movie-jobs/shiftstage/9m7RMvbn17cqVAMbPNvVFo5wuAnA). All 6,269 tests, lint, build, standalone TypeScript and thirty readiness checks passed. Post-deployment read-only verification at 05:04:40 UTC on September 12 preserved 131 functions/grants, 117 ledger entries, twelve schema groups and eighteen table fingerprints, including 452 storage objects. Seven deployed health/access/retirement GET checks and the live function gate passed. No SQL or business mutation was applied.

## Step 14 trigger review and profile video capacity

Read-only review of 59 application/Auth attachments, 41 trigger-return functions and one public event-trigger function found the intended fifty-video function unattached. Production has 34 videos and no profile above fifty; native reproduction admits 51 reservations. The inspected plan is `step-14-trigger-review.md`, including the full attachment inventory, transaction/side-effect review and explicit Step15/16/17/19 handoffs. Preserve the intentionally superseded legacy earnings trigger and managed event triggers.

The narrow additive correction replaces one invoker function and adds its AFTER INSERT/UPDATE attachment. It handles reservation, restoration, feed-to-profile and ownership transitions; existing-slot updates and idempotent conflict retries remain allowed. No existing row, browser grant, visibility or ownership changes. Thirty-nine behavioral cases and twenty guarded-deployment cases pass. The guard checks exact current schema/access, bounded locks, twenty table fingerprints, other functions/schema and prior ledger entries, then verifies the replacement and records the exact source. Production read-only preconditions passed at 05:25:31 UTC. Independent hosted-session tests remain deferred.
