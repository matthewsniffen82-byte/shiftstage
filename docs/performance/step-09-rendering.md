...
Externalize only the main stylesheet to `/outputs/live-shell.css?v=<release hash>` at its **exact original cascade position**. Keep every declaration, other style block, link order, script and UI behavior. The stylesheet URL retains the document's `/outputs/` base for relative URLs; current CSS image references are absolute or embedded. Matching versions receive immutable browser caching; missing/obsolete versions revalidate, following the existing live-script pattern.
