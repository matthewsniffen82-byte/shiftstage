...
The static follow-up was pushed as `ac451c674a64af33753cf2d1c178b1bea285ca94` and Vercel succeeded. Live CSS bytes/cache headers and all 3,486 CSSOM rules matched; roles, playback and health passed. Cold home/venue transfers dropped about 14 KB, but FCP medians remained variable/slower (home 1,608 ms, venues 1,480 ms, TV 1,776 ms), with higher measured document TTFB as well. The waterfall still showed critical CSS discovery 150–290 ms after the first document byte. A final scoped follow-up adds one HTTP Link preload for the exact same content-versioned stylesheet. The stylesheet link remains in its original cascade position; browser request counts are checked for deduplication. No other CSS/font/media preload is added. [MDN documents this early-discovery use of Link preload headers](https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Link).

The preload follow-up preserved upstream output-encoding protection (96745f70). All 2,768 automated tests, TypeScript, lint, migration guard and the production build passed on the combined release.
