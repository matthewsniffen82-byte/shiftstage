...
Use the three-dot options menu to choose **Pin** or **Unpin** in the dancer dashboard's Profile & media section, dashboard editor, and onboarding media managers. Trash controls remain separate. Full public profiles and their full-screen viewers are view-only, including when the dancer is signed in. They still display the pin order saved from the dashboard. Dashboard pin and delete actions share a busy guard, require authenticated owner-scoped server operations, and preserve existing media and order when a request fails.
