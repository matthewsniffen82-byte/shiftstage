...
The combined release includes atomic avatar publication, the reserved-link SQL correction and the separately requested desktop metric alignment. Both committed migrations receive guarded application and independent postflight before the new application is pushed. The canonical gate runs all automated tests, TypeScript, lint, dependency/signature verification and the production build. The final receipt also requires configured artifact inspection, matching local and remote main, exact successful Vercel status and hosted health checks.
