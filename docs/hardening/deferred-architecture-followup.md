...
Hosted deployment uses the pinned npm production build. Tests and other validation are selected for the changed behavior before publishing, rather than rerunning the complete release suite inside every hosting build. The full `verify:release` command remains available when specifically requested.
