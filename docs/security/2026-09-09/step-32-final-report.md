...
The user requested the outstanding architecture and stability work as one combined release. The avatar correction in step 31, this report, and the architecture/recovery follow-up therefore share one release. The desktop profile-header regression and subsequent metric-centering adjustment were published separately and are excluded from this release's change set. The original candidate passed the full validation cycle; integration after the user-requested pause uses focused checks under the updated repository policy. Repeated broad audits or report-only deployments are not prerequisites to the release.
