# Fictional Las Vegas pitch identities

The active Las Vegas directory uses original fictional names and locally generated SVG logos for the clearly labeled MyDancr pitch environment. The active assets are under `public/venue-logos/fictional/` and are generated deterministically with `npm run generate:fictional-venue-logos`.

No active venue card or profile references a third-party logo. Historical source assets remain outside the active branding map only to preserve repository history and are not used by the application.
