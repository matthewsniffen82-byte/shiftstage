...
export async function GET() {
  ...
  const dancerDashboardRoutingBridge = [
    ...
    "    const target = event.target && event.target.closest ? event.target.closest(\"#dancerLoginBtn, #openDancerSignup, #dancerJoinNowBtn, #accountBtn\") : null;",
    "    if (!target) return;",
    "    let destination = \"\";",
    ...
    "    } else {",
    "      destination = target.id === \"dancerLoginBtn\" ? \"/account?role=dancer&mode=login\" : \"/account?role=dancer&mode=signup\";",
    "    }",
