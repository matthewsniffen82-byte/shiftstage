...
import { PICKUP_STATUS_LABELS, PICKUP_TRANSPORT_NOTICE, type PickupRequest, type PickupRole, type PickupVenue } from "@/src/lib/dancr/pickup-domain";
...
function Inbox({ role }: { role: PickupRole }) {
  ...
  useEffect(() => {
    ...
    void requestPickupJson(`/api/pickups?${query}`, { signal: controller.signal }).then(data => {
      if (!controller.signal.aborted) { setRequests(data.requests); setMore(data.hasMore); }
    }).catch(failure => { if (!controller.signal.aborted) { setRequests([]); setError(failure instanceof Error ? failure.message : "Unable to load pickup requests."); } })
  ...
  async function loadMore() {
    ...
    try {
      const data = await requestPickupJson(`/api/pickups?${query}&offset=${requests.length}`);
      if (mounted.current && expectedQuery === queryRef.current) { setRequests(current => [...new Map([...current, ...data.requests].map(r => [r.id, r])).values()]); setMore(data.hasMore); }
    } catch (failure) { if (mounted.current) setError(failure instanceof Error ? failure.message : "Unable to load more pickups."); }
    finally { paginationLock.current = false; if (mounted.current) setLoading(false); }
  }
  ...
  return <section className="pickup-card">
    ...
    {role === "venue" && <details className="pickup-details"><summary>Club Pickup settings</summary><p>{PICKUP_TRANSPORT_NOTICE}</p>
      <p>Only enable pickup if your venue can manage requests. Disabling stops new requests; existing conversations remain available. Each manager must accept the chat notice before reading or sending messages.</p>
      {venues.map(venue => <div key={venue.id}><strong>{venue.name}</strong><p>{venue.club_pickup_enabled ? "Pickup enabled" : "Pickup disabled"}</p><button disabled={saving || (!venue.eligible && !venue.club_pickup_enabled)} onClick={() => void setEnabled(venue)}>{venue.club_pickup_enabled ? "Disable Club Pickup" : "Enable Club Pickup"}</button>{!venue.eligible && <p>Publish your verified venue page before enabling pickup.</p>}</div>)}
    ...
    {!loading && !error && !requests.length && <p>No pickup requests in this group.</p>}
