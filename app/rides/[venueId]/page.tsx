...
export default async function FreeRidePage({ params, searchParams }: {
  ...
  const { data: venue, error } = await admin.from("venues").select("id, name, slug, owner_user_id, address, city, state, club_pickup_available")
  ...
  return <TransportationClient deal={deal ? toPublicClubDeal(deal) : undefined}
    ...
    initialTransportation="club_shuttle" pickupAvailable={venue.club_pickup_available === true}
