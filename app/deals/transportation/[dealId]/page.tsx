...
export default async function TransportationPage({ params, searchParams }: {
  ...
  if (!deal) notFound();
  const { data: venue, error } = await admin.from("venues").select("name, slug, phone, owner_user_id")
  ...
  return <TransportationClient deal={toPublicClubDeal(deal)} venue={{ id: deal.venueId, name: venue.name, slug: venue.slug }} shuttleAvailable={shuttleAvailable}
