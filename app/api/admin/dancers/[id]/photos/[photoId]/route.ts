...
export async function DELETE(request: Request, context: RouteContext) {
  try {
    ...
    if (!UUID_PATTERN.test(id || "") || !UUID_PATTERN.test(photoId || "")) {
      return NextResponse.json({ ok: false, error: "Invalid dancer or photo ID." }, { status: 400 });
    }
    ...
    const deleted = await deleteAdminDancerPhoto(admin, {
      dancerId: id,
      targetId: photoId,
      adminId: user.id,
    });
    if (!deleted) {
      return NextResponse.json({ ok: false, error: "Dancer photo not found." }, { status: 404 });
    }
