...
export default function AdminSalesAgentPanel({ onActionConfirmed }: { onActionConfirmed: (message: string) => void }) {
  ...
  function natsAction(event: SyntheticEvent, action: string, target: Row, resolution?: string) {
    const reason = window.prompt(action.includes("verify") ? "Confirm the payout account and required tax forms were checked in the payout portal. Enter an audit note:" : "Enter a payout account audit note:");
    if (!reason?.trim()) { event.preventDefault(); return; }
    return submit(event, { action, agentId: target.agent_id, exportId: target.id, resolution, reason: reason.trim() }, "Agent commission records updated.");
  }

  return <section className="sales-workspace"><header><span>Venue sales</span><h2>Sales agents and commission payouts</h2>
    ...
    <div className="sales-metrics"><Metric label="Active agents" value={program?.metrics?.activeAgents || 0} /><Metric label="Attributed clubs" value={program?.metrics?.attributedVenues || 0} />
      <Metric label="Waiting on clubs" value={money(program?.metrics?.pendingVenuePaymentCents)} /><Metric label="Ready to send for payout" value={money(program?.metrics?.payableCents)} />
    ...
    {program?.nats?.selected ? <section className="sales-list"><h3>Agent payout accounts</h3><p>Activate only after matching the payee in the payout portal and confirming approval of their required tax forms. MyDancr never stores W-9 or identity-document contents.</p>
    ...
    {program?.nats?.selected ? <section className="sales-list"><h3>Agent commission records sent for payout</h3>{natsExports.filter((row: Row) => ["failed", "reconciliation_required"].includes(row.status)).map((row: Row) => <article key={row.id}><div><strong>{money(row.amount_cents)} · {row.status.replaceAll("_", " ")}</strong><small>{payoutCopy(String(row.last_error || ""))}</small></div><div className="row-actions">{row.status === "failed" ? <button disabled={working} onClick={(event) => natsAction(event, "retry_nats_agent_export", row)}>Retry rejected</button> : <><button disabled={working} onClick={(event) => natsAction(event, "reconcile_nats_agent_export", row, "confirmed_exported")}>Confirmed</button><button disabled={working} onClick={(event) => natsAction(event, "reconcile_nats_agent_export", row, "confirmed_not_exported")}>Not exported</button></>}</div></article>)}</section> : null}
