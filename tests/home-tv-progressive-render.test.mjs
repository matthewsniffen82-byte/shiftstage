...
function fixture() {
  ...
  const context = vm.createContext({
    activeTab: 'tv', homeTvFeedVideos: Array.from({ length: 22 }, (_, i) => ({ id: String(i) })),
  ...
  vm.runInContext(fn('hydrateHomeTvFeedSlide') + '\n' + fn('finishHomeTvFeedCards'), context);
...
test('initial TV render builds three cards and preserves all ordered scroll slots', () => {
  ...
  assert.equal(f.built.length, 22);
});
