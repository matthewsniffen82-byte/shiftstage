...
test("the live dancer profile close control exits shared links and remains touchable", () => {
  ...
  assert.match(
    homeSource,
    /@media \(max-width: 720px\) \{[\s\S]*?#profileBackdrop \.profile-modal \{[\s\S]*?--profile-report-clearance: max\(84px, calc\(16px \+ env\(safe-area-inset-bottom, 0px\)\)\);[\s\S]*?overscroll-behavior-y: none;[\s\S]*?scroll-padding-bottom: var\(--profile-report-clearance\) !important;[\s\S]*?#profileBackdrop \.modal-body \{[\s\S]*?padding-bottom: 0 !important;[\s\S]*?#profileBackdrop \.modal-actions \{[\s\S]*?scroll-margin-bottom: var\(--profile-report-clearance\);[\s\S]*?padding-bottom: var\(--profile-report-clearance\) !important;/,
  );
