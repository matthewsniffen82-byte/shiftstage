...
test("mobile full profiles keep identity, analytics, and close control on one compact plane", () => {
  ...
  assert.match(
    aesthetic,
    /Keep the stage name and verified badge together[\s\S]*?\.profile-modal-summary \.modal-identity \{[\s\S]*?position: relative !important;[\s\S]*?left: 0 !important;[\s\S]*?width: 100% !important;[\s\S]*?max-width: none !important;[\s\S]*?justify-self: stretch !important;[\s\S]*?#profileBackdrop #profileModal \.profile-modal-header-metrics \{[\s\S]*?margin-left: -4px !important;[\s\S]*?#profileBackdrop #profileModal \.profile-modal-name-row \{[\s\S]*?position: absolute !important;[\s\S]*?left: calc\(\(\(100% - 22px\) \/ 6\) - 4px\) !important;[\s\S]*?width: fit-content !important;[\s\S]*?max-width: min\(128px, calc\(100% - 38px\)\) !important;[\s\S]*?padding: 0 !important;[\s\S]*?transform: translateX\(-50%\) !important;[\s\S]*?display: grid !important;[\s\S]*?grid-template-columns: 19px auto 19px !important;[\s\S]*?justify-content: center !important;/,
  );
  assert.match(
    aesthetic,
    /Keep the stage name and verified badge together[\s\S]*?\.profile-modal-name-row::before \{[\s\S]*?grid-column: 1;[\s\S]*?width: 19px;[\s\S]*?#profileBackdrop #profileModal \.profile-modal-name-row h2 \{[\s\S]*?grid-column: 2;[\s\S]*?width: auto !important;[\s\S]*?max-width: 80px !important;[\s\S]*?text-align: center !important;[\s\S]*?\.profile-modal-name-row \.profile-modal-verified \{[\s\S]*?position: static !important;[\s\S]*?grid-column: 3;[\s\S]*?flex: 0 0 19px !important;[\s\S]*?transform: none !important;/,
  );
  assert.doesNotMatch(
    aesthetic,
    /Keep the stage name and verified badge together[\s\S]*?#profileBackdrop #profileModal \.profile-modal-name-row \{[\s\S]*?left: auto !important;/,
  );
  assert.match(
    aesthetic,
