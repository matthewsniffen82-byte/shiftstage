...
test("mobile venue cards keep the violet divider without a violet header glow", () => {
  ...
  assert.match(mobileCards, /Keep the club header neutral/);
  assert.match(mobileCards, /border-bottom: 1px solid var\(--dancr-color-brand-primary-medium\)/);
  assert.doesNotMatch(mobileCards, /radial-gradient[^;]*(?:beam-violet-soft|brand-glow-soft)/);
  assert.match(mobileCards, /linear-gradient\(145deg, #151518 0%, #090a0f 64%, #101014 100%\)/);
