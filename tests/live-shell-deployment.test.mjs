...
test("development, type checking, and production builds refresh the live shell hash", () => {
  ...
  assert.equal(scripts.predev, "npm run generate:live-shell-version && npm run generate:live-shell-styles && npm run generate:static-asset-versions");
  assert.equal(scripts.pretypecheck, "npm run generate:live-shell-version && npm run generate:live-shell-styles && npm run generate:static-asset-versions");
  assert.equal(scripts.pretest, scripts.pretypecheck);
  assert.equal(scripts.prebuild, "npm run db:check-migrations && npm run generate:live-shell-version && npm run generate:live-shell-styles && npm run generate:static-asset-versions");
