...
test("the deployed root route is content-addressed to the current live HTML", () => {
  ...
  assert.match(rootRouteSource, /externalizeLiveShellAppScript\([\s\S]*?`\/live-shell\.js\?v=\$\{scriptVersion\}`/);
...
test("development, type checking, and production builds refresh the live shell hash", () => {
  ...
  assert.equal(scripts.predev, "node scripts/check-runtime-dependencies.mjs && npm run generate:live-shell && npm run generate:live-shell-version && npm run generate:live-shell-styles && npm run generate:static-asset-versions");
  assert.equal(scripts.pretypecheck, "node scripts/check-runtime-dependencies.mjs && npm run generate:live-shell-version && npm run generate:live-shell-styles && npm run generate:static-asset-versions");
  assert.equal(scripts.pretest, scripts.pretypecheck);
  assert.equal(scripts.prebuild, "node scripts/check-runtime-dependencies.mjs && npm run check:live-shell-source && npm run db:check-migrations && npm run generate:live-shell-version && npm run generate:live-shell-styles && npm run generate:static-asset-versions");
