...
test("development, type checking, and production builds refresh the live shell hash", () => {
  ...
  assert.equal(scripts.predev, "node scripts/check-runtime-dependencies.mjs && npm run generate:live-shell-version && npm run generate:live-shell-styles && npm run generate:static-asset-versions");
  ...
  assert.equal(scripts.prebuild, "node scripts/check-runtime-dependencies.mjs && npm run db:check-migrations && npm run generate:live-shell-version && npm run generate:live-shell-styles && npm run generate:static-asset-versions");
