...
test("existing installed sessions refresh onto the current production shell", () => {
  assert.match(liveAppSource, /register\("\/sw\.js\?v=electric-white-avatar-v1", \{ updateViaCache: "none" \}\)/);
  assert.match(liveAppSource, /registration\.update\(\)/);
  assert.match(serviceWorkerSource, /dancr-sw-release: electric-white-avatar-v1/);
