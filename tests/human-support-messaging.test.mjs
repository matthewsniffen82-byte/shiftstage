...
test("existing installed sessions refresh onto the current production shell", () => {
  assert.match(liveAppSource, /register\("\/sw\.js\?v=all-dancer-avatar-solid-white-v2", \{ updateViaCache: "none" \}\)/);
  assert.match(liveAppSource, /registration\.update\(\)/);
  assert.match(serviceWorkerSource, /dancr-sw-release: all-dancer-avatar-solid-white-v2/);
