...
test("manual admin finance writes use one dedicated action boundary", () => {
  ...
  assert.doesNotMatch(route, /finance-admin-dispatch/);
...
test("retired Bitsafe reconciliation is absent from admin finance actions", () => {
  assert.doesNotMatch(actions, /bitsafe|yoursafe|reconcileBitsafe/i);
});
