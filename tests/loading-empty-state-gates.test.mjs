...
test("secondary account panels do not claim data is empty while requests are loading", () => {
  ...
  assert.match(
    venueNfcPanel,
    /const isRosterLoading = isLoading && !workingOnly && !activeAffiliations\.length;[\s\S]*?\{!isRosterLoading && !rosterEntries\.length \? <p>[\s\S]*?No dancers have used this venue's dancer check-in sticker yet\./,
  );
  assert.match(
    venueTeamPanel,
