...
test("homepage navigation stays anchored to the CSS viewport in Samsung Browser", () => {
  ...
  assert.match(
    homeSource,
    /@media \(max-width: 720px\) \{[\s\S]*?body \{[\s\S]*?padding-bottom: calc\(92px \+ env\(safe-area-inset-bottom\)\)/,
  );
  assert.match(
    homeSource,
...
test("routed-page navigation uses the same viewport-safe bottom offset", () => {
  ...
  assert.match(
    globalNavigation,
    /body \{[\s\S]*?padding-bottom: calc\(92px \+ env\(safe-area-inset-bottom\)\) !important/,
  );
  assert.match(
    globalNavigation,
