...
test("profile grid photos use the native responsive image path used by dancer discovery", () => {
  ...
  assert.match(
    profileThumbRenderer,
    /<img class="portrait \$\{escapeHtml\(item\.photoClass\)\} has-custom-photo" \$\{photoAttrs\}[^>]*sizes="\(max-width: 720px\) calc\(\(100vw - 6px\) \/ 3\), 250px"[^>]*width="360" height="504"[^>]*loading="\$\{total <= PROFILE_MEDIA_PAGE_SIZE \|\| galleryIndex < 6 \? "eager" : "lazy"\}"[^>]*decoding="async"[^>]*draggable="false"[^>]*data-image-state="loading"[^>]*>/,
  );
