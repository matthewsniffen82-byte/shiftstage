...
test("dancer setup no longer offers commission enrollment", () => {
  for (const source of [dashboard, liveApp]) {
    assert.doesNotMatch(source, /Commissions start only after|Earlier redemptions do not earn commissions|Sign up for commission payouts/);
  }
});
