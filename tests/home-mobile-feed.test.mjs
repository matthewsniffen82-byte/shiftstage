...
test("all five mobile destinations share a violet-only card border without coloring the gutters", () => {
  assert.match(
    homeSource,
  ...
  assert.match(
    homeSource,
    /@media \(max-width: 679px\) \{[\s\S]*?#results \{[\s\S]*?--home-card-edge-violet: rgba\(139,92,246,.82\);[\s\S]*?--home-card-glow: transparent;/,
  );
  assert.match(
    homeSource,
    /@supports \(-webkit-touch-callout: none\) \{[\s\S]*?#results \{[\s\S]*?--home-card-edge-violet: rgba\(139,92,246,.34\);[\s\S]*?--home-card-glow: transparent;/,
  );
  assert.match(
    homeSource,
    /#results\.home-dancer-grid > \.home-dancer-grid-card,[\s\S]*?#results\.home-tv-feed > \.home-tv-feed-slide,[\s\S]*?#results\.home-discovery-feed\.home-venue-discovery-feed > \.home-venue-discovery-slide,[\s\S]*?#results\.card-grid > \.dancer-card\.trending-card,[\s\S]*?#results\.venue-card-grid > \.venue\.venue-card \{[\s\S]*?linear-gradient\(145deg,var\(--home-card-edge-violet\),var\(--home-card-edge-violet\)\) border-box !important;[\s\S]*?0 0 22px var\(--home-card-glow\) !important;/,
  );
