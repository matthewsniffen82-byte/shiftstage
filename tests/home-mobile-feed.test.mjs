...
test("venue inline cards use production venue, schedule, revenue, and customer action data", () => {
  ...
  assert.match(
    homeSource,
    /function homeVenueDiscoveryQrMarkup\(venue\)[\s\S]*?venue\?\.id && venue\.activeDeal\?\.id[\s\S]*?data-card-action-slot="qr"[\s\S]*?data-club-deal-state="available"[\s\S]*?data-club-deal-cta[\s\S]*?actionButtonLabel\("qr", offerCount > 1 \? "Club Deals" : "Club Deal"\)[\s\S]*?data-club-deal-state="unavailable"[\s\S]*?data-card-qr-label="Club Deal unavailable"[\s\S]*?actionButtonLabel\("qr", "Club Deals"\)/,
  );
...
test("Working Now dancer grid cards expose a functional cashier-tap Club Deal action", () => {
  ...
  assert.match(homeSource, /selectButton\.textContent = "Ready at Cashier ✓"/);
  assert.match(homeSource, /Unlock your phone and tap the MyDancr cashier sticker\./);
  assert.match(homeSource, /You can close MyDancr now\./);
...
test("bottom navigation keeps every destination on one uniform baseline", () => {
  ...
  assert.match(
    homeSource,
    /#discoveryTabs \.tab,\s*#discoveryTabs \.home-bottom-tv \{[\s\S]*?--home-nav-accent: rgba\(244,242,248,.86\)[\s\S]*?--home-nav-accent-soft: rgba\(244,242,248,.78\)[\s\S]*?--home-nav-active: #f5f3ff[\s\S]*?--home-nav-active-violet-core: rgba\(124,58,237,.96\)[\s\S]*?--home-nav-active-violet-glow: rgba\(124,58,237,.58\)[\s\S]*?--home-nav-active-violet-depth: rgba\(49,46,129,.72\)/,
  );
  assert.match(
    homeSource,
