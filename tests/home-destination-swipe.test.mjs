...
test("mobile destinations can be changed with an easy deliberate horizontal swipe", () => {
  ...
  assert.match(
    homeSource,
    /function activateHomeDestination\(nextTab, options = \{\}\) \{[\s\S]*?activeTab = nextTab;[\s\S]*?render\(\);[\s\S]*?options\.scroll !== false[\s\S]*?requestAnimationFrame\(\(\) => \{[\s\S]*?nextTab === "tv" && homeTvFeedUsesSnapViewport\(\)[\s\S]*?focusHomeResults\(\)/,
  );
  assert.match(
    homeSource,
