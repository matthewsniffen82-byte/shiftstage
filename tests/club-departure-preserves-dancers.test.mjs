...
import { createClubDepartureDatabase, clubDepartureMigration, dancerRetapMigration, restoreRetapPermissionMigration } from "./helpers/club-departure-database.mjs";
...
test("restoring club permission covers prior roster removals without changing membership or accounts", async () => {
  ...
  try {
    ...
    assert.equal((await beforeDb.query("select reentry_blocked from public.venue_dancer_affiliations")).rows[0].reentry_blocked, false);
    const accounts = (await beforeDb.query("select * from public.app_users order by id")).rows;
    await beforeDb.exec(restoreRetapPermissionMigration);
    assert.deepEqual((await beforeDb.query("select status,reentry_blocked from public.venue_dancer_affiliations")).rows, [{status:"revoked",reentry_blocked:true}]);
