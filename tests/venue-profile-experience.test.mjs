...
test("the canonical in-app venue page is dedicated to the selected club and its live production data", () => {
  ...
  assert.match(venueDetail, /const hasPostedHours = operatingStatus\.state !== "unknown"/);
  assert.match(venueDetail, /venue-status-grid\$\{hasPostedHours \? "" : " without-hours"\}/);
  assert.match(venueDetail, /\$\{hasPostedHours \? `[\s\S]*?venue-operating-status is-\$\{operatingStatus\.state\}">\$\{escapeHtml\(operatingStatus\.label\)\}[\s\S]*?: ""\}/);
  assert.doesNotMatch(venueDetail, />Not posted</);
  ...
  assert.match(venueDetail, /class="venue-status-grid\$\{hasPostedHours \? "" : " without-hours"\}" aria-label="Tonight at \$\{escapeHtml\(details\.name\)\}"[\s\S]*?venue-operating-summary[\s\S]*?venue-status-kicker">Hours[\s\S]*?\$\{quickStats\}/);
...
test("venue profile hierarchy stays compact and carries the restrained venue brand signature", () => {
  ...
  assert.match(refinement, /\.venue-status-grid\.without-hours \{[\s\S]*?grid-template-columns: repeat\(2, minmax\(0, 1fr\)\);/);
