...
test("venue profile hierarchy stays compact and carries the restrained venue brand signature", () => {
  ...
  assert.match(refinement, /\.venue-primary-actions > \.venue-detail-uber \{[\s\S]*?border: 1px solid var\(--dancr-color-brand-primary-medium\) !important;[\s\S]*?var\(--dancr-color-surface-raised\) 82%[\s\S]*?backdrop-filter: blur\(16px\)/);
  ...
  assert.match(refinement, /\.venue-primary-actions > \.venue-detail-uber \.action-icon \{[\s\S]*?color: var\(--dancr-color-brand-core\) !important;[\s\S]*?filter: none !important;/);
...
test("venue profiles stay full-screen with X dismissal and the shared floating navigation", () => {
  ...
  assert.match(
    aesthetic,
    /\.venue-detail-close \{[\s\S]*?position: absolute !important;[\s\S]*?top: 12px !important;[\s\S]*?right: 8px !important;/,
  );
  assert.match(
    liveApp,
