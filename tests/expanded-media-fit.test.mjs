...
test("expanded profile photos and videos fit completely without changing thumbnail crops", () => {
  assert.match(live, /\.profile-photo-viewer-slide-image \{[^}]*background-size: contain !important;/);
  assert.match(live, /\.profile-tv-viewer-video \{[^}]*object-fit: contain;/);
  assert.match(profile, /\.profile-media-viewer\.is-photo \.profile-media-viewer-slide > img \{ object-fit: contain; \}/);
  assert.match(profile, /\.profile-media-viewer\.is-video \.profile-media-viewer-slide > video \{ object-fit: contain; \}/);
  assert.match(tvStrip, /\.tv-video-viewer-stage > video \{[^}]*object-fit: contain;/);
...
test("signed-in full-profile previews fit media within the dynamic viewport", () => {
  ...
  assert.match(dashboard, /\.dancer-profile-preview-overlay \.profile-media-viewer-slide > img, \.dancer-profile-preview-overlay \.profile-media-viewer-slide > video \{[^}]*min-width:0; min-height:0;[^}]*object-fit:contain;/);
...
test("expanded TV contains video in native and fallback fullscreen without a large-viewport minimum", () => {
  ...
  assert.match(live, /#results\.home-tv-feed:is\(:fullscreen, :-webkit-full-screen, \.is-fullscreen-feed\) \.home-tv-feed-video \{\s*object-fit: contain;\s*\}/);
});
