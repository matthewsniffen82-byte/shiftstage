...
test("public social icons render without a visible heading or published handles", () => {
  ...
  assert.match(profilePage, /\.profile-social-section \{ min-height: 0;[\s\S]*?margin: 0 0 22px;[\s\S]*?padding: 0;/);
  assert.match(profilePage, /\.social-list \{ width: fit-content;[\s\S]*?flex-wrap: wrap;[\s\S]*?justify-content: center;[\s\S]*?gap: 6px;/);
  assert.match(profilePage, /\.social-list a \{[\s\S]*?width: 44px;[\s\S]*?height: 44px;[\s\S]*?justify-content: center;/);
  assert.match(profilePage, /\.social-list a::before \{[\s\S]*?inset: 0;[\s\S]*?border: 1px solid rgba\(226,232,240,\.14\);[\s\S]*?border-radius: 50%;/);
  assert.match(profilePage, /\.social-list a svg \{ position: relative; z-index: 1; width: 16px; height: 16px;/);
});
