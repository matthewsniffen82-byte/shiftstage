...
test("the public dancer profile keeps a compact identity that scrolls with the whole profile", () => {
  ...
  assert.match(profilePage, /\.profile-titlebar \{[\s\S]*?min-height: 64px;[\s\S]*?grid-template-columns: minmax\(120px, \.95fr\) minmax\(150px, 1\.05fr\) 94px;[\s\S]*?gap: 6px;/);
...
test("profile videos stay passive and duration-free in the grid, then open the complete full-screen player", () => {
  ...
  assert.match(profileCarousel, /index === viewerIndex[\s\S]*?void playViewerVideo\(video\)[\s\S]*?video\.pause\(\)/);
