...
export const tapSchema=JSON.parse(readFileSync(new URL('../fixtures/dancer-tap-lock-schema.json',import.meta.url),'utf8'));
...
export const lockMigrationPath='supabase/migrations/20260910172000_serialize_dancer_nfc_enrollment.sql';
export const lockMigration=readFileSync(new URL('../../'+lockMigrationPath,import.meta.url),'utf8').replace(/\r\n/g,'\n');
...
export async function createTapDatabase({migrate=true}={}){
 ...
 try{
  ...
  // Auth identities and peripheral tables are synthetic projections; all eleven current target tables and twelve triggers retain the captured schema.
  ...
  await pg.exec('create table public.gallery_media_reference_history(id bigint generated always as identity,source_kind text,source_id uuid,profile_id uuid,storage_path text,event_kind text);create table public.gallery_storage_retirements(storage_path text primary key);create table public.image_moderation_records(id uuid primary key,final_storage_path text)');
  await pg.exec(tapSchema.galleryFamily.definition);
  // Gallery support tables above are explicit projections; the actual gallery
  // trigger definitions and family helper are retained in this NFC fixture.
  ...
  await pg.exec(tapMigration);
  if(migrate)await pg.exec(lockMigration);
  return pg;
 }catch(error){await pg.close();throw error;}
}
export async function seedTapDatabase(pg,{migrate=true}={}){
 ...
 await pg.exec('truncate public.gallery_media_reference_history,public.gallery_storage_retirements,public.image_moderation_records,'+tapTables.map(t=>'public.'+t).concat(['auth.users','public.dancer_profile_slug_aliases','public.venue_team_members','public.club_deals']).join(','));
...
export async function tapSnapshot(pg){
 const out={};for(const table of [...tapTables,'gallery_media_reference_history','gallery_storage_retirements'])out[table]=(await pg.query('select to_jsonb(t) row from public.'+table+' t order by to_jsonb(t)::text')).rows.map(r=>r.row);return out;
}
