...
import * as mobile from '../src/lib/dancr/video-mobile-playback.ts';
...
vm.runInNewContext(compile(source),{exports,require:name=>name==='./media-watermark'?media:name==='./adaptive-video-manifest'?adaptive:name==='./video-mobile-playback'?mobile:{},console:{info:value=>messages.push(JSON.parse(value))}});
let db;
...
function harness(options={}){
 const calls=[],path=options.path||pathFor('mp4'),files=options.files||new Set(['mydancr-tv-videos/'+path,'dancer-photos/'+media.myDancrTvPosterStoragePath(path),'mydancr-tv-videos/'+media.archivedOriginalStoragePath('mydancr-tv-videos',path),'mydancr-tv-videos/'+mobile.mobileVideoStoragePath(path)]);
 let removes=0;
...
test('actual owner removal confirms native hidden state before deleting the video, poster, archive and mobile copy',async()=>{
 const before=await importSnapshot(db),h=harness(),result=await h.run();
 assert.deepEqual(JSON.parse(JSON.stringify(result)),{id:video,status:'hidden'});assert.deepEqual(h.calls.map(c=>c.kind),['read','update','remove','remove','remove','remove']);assert.equal(h.files.size,0);assert.equal((await state()).venue_featured,false);
...
for(const [mime,ext]of [['video/mp4','mp4'],['video/webm','webm'],['video/quicktime','mov']])test(mime+' derives the owned video, poster and private original paths',async()=>{
 ...
 assert.deepEqual(h.calls.filter(c=>c.kind==='remove').map(c=>[c.bucket,c.path]),[['mydancr-tv-videos',pathFor(ext)],['dancer-photos',media.myDancrTvPosterStoragePath(pathFor(ext))],['mydancr-tv-videos','__originals/'+pathFor(ext)],['mydancr-tv-videos',mobile.mobileVideoStoragePath(pathFor(ext))]]);
});
for(const [label,actor,id]of [['foreign owner',fixtureId(2),video],['missing record',owner,fixtureId(99)]])test(label+' cannot mutate metadata or storage',async()=>{
 const before=await importSnapshot(db),h=harness();await assert.rejects(h.run(actor,id));assert.equal(h.calls.length,1);assert.equal(h.files.size,4);assert.deepEqual(await importSnapshot(db),before);
});
for(const [key,value]of [['id',fixtureId(21)],['submitted_by',fixtureId(2)],['dancer_id','invalid'],['updated_at',null],['updated_at','invalid'],['storage_path','somebody/else/video.mp4'],['storage_mime','video/unknown']])test('unconfirmed selected '+key+' fails before writes',async()=>{
 const row=await state(),h=harness({readData:{...row,[key]:value}}),before=await importSnapshot(db);await assert.rejects(h.run());assert.equal(h.calls.length,1);assert.equal(h.files.size,4);assert.deepEqual(await importSnapshot(db),before);
});
for(const key of ['readError','updateError','lostWriteReply'])test(key+' never deletes storage or logs successful removal',async()=>{
 const h=harness({[key]:true});await assert.rejects(h.run());assert.equal(h.files.size,4);assert.ok(h.calls.every(c=>c.kind!=='remove'));assert.equal(messages.length,0);
...
for(const receipt of [null,undefined,[],{},false])test('malformed database receipt '+String(receipt)+' preserves every object',async()=>{
 const h=harness({receipt});await assert.rejects(h.run());assert.equal(h.files.size,4);assert.ok(h.calls.every(c=>c.kind!=='remove'));
});
for(const [key,value]of [['id',fixtureId(21)],['submitted_by',fixtureId(2)],['dancer_id',fixtureId(11)],['status','approved'],['storage_path','different.mp4'],['storage_mime','video/webm'],['venue_featured',true]])test('mismatched database receipt '+key+' preserves every object',async()=>{
 const h=harness({transformReceipt:row=>({...row,[key]:value})});await assert.rejects(h.run());assert.equal(h.files.size,4);assert.ok(h.calls.every(c=>c.kind!=='remove'));
});
for(const [column,value]of [['status','submitted'],['storage_path','changed/path.mp4'],['storage_mime','video/webm'],['submitted_by',fixtureId(2)],['updated_at','2026-01-01T01:02:03.123456Z']])test('concurrent '+column+' change invalidates the native final update before cleanup',async()=>{
 const h=harness({beforeUpdate:()=>db.query('update public.mydancr_tv_videos set '+column+'=$1 where id=$2',[value,video])});await assert.rejects(h.run());assert.equal(h.files.size,4);assert.notEqual((await state()).status,'hidden');assert.ok(h.calls.every(c=>c.kind!=='remove'));
});
...
for(const key of ['storageThrowAt','storageErrorAt','lostStorageReplyAt'])for(const stage of [1,2,3,4])test(key+' at stage '+stage+' remains hidden and an explicit retry safely completes',async()=>{
...
test('a lost database reply can be retried against the hidden record with its new native version',async()=>{
 const h=harness({lostWriteReply:true});await assert.rejects(h.run());assert.equal(h.files.size,4);const retry=harness({files:h.files});await retry.run();assert.equal(h.files.size,0);
});
