...
async function database(){
 ...
 try{
  ...
  // invokes these seven actual functions; business semantics have separate tests.
...
test('release checker reads the actual reviewed interfaces without invoking functions or changing records/access',async()=>{
 const pg=await database();try{const before=await snapshot(pg),result=await pg.exec(sql);const receipt=result.flatMap(r=>r.rows||[]).find(r=>r.function_security)?.function_security;assert.equal(receipt.ok,true);assert.equal(receipt.read_only,'on');assert.equal(receipt.public_functions,8);assert.equal(receipt.anonymous_interfaces,4);assert.equal(receipt.authenticated_interfaces,7);assert.equal(receipt.application_functions_invoked,false);assert.deepEqual(await snapshot(pg),before);}finally{await pg.close();}
});
const failures=[
