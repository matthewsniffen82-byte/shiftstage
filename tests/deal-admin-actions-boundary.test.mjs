...
test("admin deal routes retire venue collections and delegate fraud writes", () => {
  assert.match(route, /from "@\/src\/lib\/dancr\/deal-admin-actions"/);
  assert.doesNotMatch(route, /settleDealRevenueEvent/);
...
test("admin contract deal writes are abortable and serialized", () => {
  const manager = adminClient.match(/function AdminClubDealManager[\s\S]*?(?=function OperationsOverview)/)?.[0] || "";
...
test("admin deal activity rejects stale filters and serializes redemption mutations", () => {
  ...
  assert.equal((manager.match(/const request = beginDealActivityAction\(\)/g) || []).length, 1);
