...
test("dancer attribution is locked to a verified shift when the cashier NFC tap is confirmed", () => {
  ...
  assert.match(liveApp, /function selectDealPassForNfc[\s\S]*?attributionToken: pass\.sourceType === "dancer_profile" \? pass\.attributionToken \|\| "" : ""/);
});
...
test("legacy uploaded QR images cannot masquerade as commission-bearing MyDancr QR codes", () => {
  ...
  assert.match(
    venueOfferHelper,
    /venue\?\.activeDeal[\s\S]*?<button class="venue-detail-club-deal-cta"[\s\S]*?data-club-deal-cta="\$\{encodeDealPass\(config\)\}"[\s\S]*?Club Deal/,
  );
  ...
  assert.match(liveApp, /function homeVenueDiscoveryQrMarkup\(venue\)[\s\S]*?venue\.activeDeal\?\.id[\s\S]*?data-club-deal-cta[\s\S]*?data-card-qr-label="Free entry unavailable"/);
  const venueQrHelper =
    liveApp.match(
