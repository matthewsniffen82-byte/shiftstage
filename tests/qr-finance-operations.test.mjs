...
test("Stripe webhook idempotently reconciles invoices, payout accounts, and reversed transfers", () => {
  ...
  assert.ok(
    webhook.indexOf('if (!signature)') < webhook.indexOf('getServerEnv("STRIPE_WEBHOOK_SECRET")'),
...
test("daily automation and every production finance dashboard are wired", () => {
  ...
  assert.deepEqual(JSON.parse(vercel).crons.filter(({ path }) => path === "/api/cron/finance"), [
    { path: "/api/cron/finance", schedule: "0 10 * * *" },
  ]);
