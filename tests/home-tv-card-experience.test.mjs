...
test("idle TV utility controls use frosted-clear glass while selected reactions keep restrained violet", () => {
  ...
  assert.match(homeSource, /dancr-aesthetic\.v1\.css\?v=82/);
});
...
test("production TV cards use the neutral-first brand palette without changing media or navigation", () => {
  ...
  assert.match(
    brandedCards,
    /\.home-tv-feed-dancer-photo,[\s\S]*?\.tv-profile-photo[\s\S]*?\)::after \{[\s\S]*?padding: 3px;[\s\S]*?conic-gradient\([\s\S]*?var\(--dancr-color-avatar-ring-magenta\)[\s\S]*?var\(--dancr-color-avatar-ring-violet\)[\s\S]*?var\(--dancr-color-avatar-ring-blue\)[\s\S]*?var\(--dancr-color-avatar-ring-core\)[\s\S]*?mask-composite: exclude;/,
  );
  assert.match(
    brandedCards,
