...
test("idle TV utility controls use frosted-clear glass while selected reactions keep restrained violet", () => {
  ...
  assert.match(homeSource, /dancr-aesthetic\.v1\.css\?v=84/);
});
...
test("production TV cards use the neutral-first brand palette without changing media or navigation", () => {
  ...
  assert.match(
    brandedCards,
    /\.home-tv-feed-dancer-photo,[\s\S]*?\.tv-profile-photo[\s\S]*?\)::after \{[\s\S]*?border: 3px solid transparent;[\s\S]*?border-top-color: var\(--dancr-color-avatar-ring-magenta\);[\s\S]*?border-right-color: var\(--dancr-color-avatar-ring-violet\);[\s\S]*?border-bottom-color: var\(--dancr-color-avatar-ring-blue\);[\s\S]*?border-left-color: var\(--dancr-color-avatar-ring-core\);/,
  );
  assert.match(
    brandedCards,
