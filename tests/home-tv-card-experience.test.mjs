...
test("idle TV utility controls use frosted-clear glass while selected reactions keep restrained violet", () => {
  ...
  assert.match(homeSource, /dancr-aesthetic\.v1\.css\?v=71/);
});
...
test("production TV cards use the neutral-first brand palette without changing media or navigation", () => {
  ...
  assert.match(brandedCards, /color: var\(--dancr-color-verified-check\) !important/);
  assert.match(brandedCards, /background: var\(--dancr-verified-badge-background\) !important/);
  assert.match(brandedCards, /box-shadow: var\(--dancr-shadow-verified\) !important/);
