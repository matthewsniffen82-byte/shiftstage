...
const reserve = async (id = user) => (await db.query('select public.reserve_dancer_age_verification($1) as value', [id])).rows[0].value;
...
test('anonymous and authenticated users cannot read, forge, activate, or reserve verification records', async () => {
  for (const role of ['anon', 'authenticated']) {
    ...
    for (const sql of ['select * from public.dancer_age_verifications', 'update public.dancer_age_verification_settings set enabled=false', 'select public.activate_dancer_age_verification()', `select public.reserve_dancer_age_verification('${user}')`]) {
...
test('verified adults can publish, and a later failed decision hides the profile again', async () => {
  await db.exec(`update public.dancer_age_verifications set status='verified',session_id=gen_random_uuid(),workflow_id='live',verified_at=now();
