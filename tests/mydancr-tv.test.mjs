...
test("approved videos appear on real dancer and venue pages", () => {
  ...
  assert.match(videoStrip, /<video[\s\S]*?autoPlay[\s\S]*?controls[\s\S]*?src=\{activeVideo\.videoUrl\}/);
  assert.match(videoStrip, /requestFullscreen\(\)/);
  ...
  assert.match(liveApp, /card\.addEventListener\("click", \(\) => openProfileTvViewer\(item, profile\.name, scheduleLabel\)\)/);
  assert.match(liveApp, /id="profileTvViewer"[\s\S]*?role="dialog"[\s\S]*?profile-tv-viewer-video[\s\S]*?controls loop playsinline/);
  assert.match(liveApp, /async function enterProfileTvFullscreen\(\)[\s\S]*?requestFullscreen\(\)/);
