...
test("mobile venue controls use a compact premium glass hierarchy without a backing slab", () => {
  ...
  assert.match(hierarchy, /\.venue-card-deals-action\.is-available \{[\s\S]*?rgba\(8, 37, 23, 0\.7\)/);
...
test("mobile venue identity stays compact, anchored, and single-line", () => {
  ...
  assert.match(hierarchy, /grid-template-rows: 88px minmax\(100px, 1fr\) 56px 56px !important;/);
  ...
  assert.match(hierarchy, /\.venue-card-primary-action:is\(\.is-inactive-demo, \.is-travel-unavailable\) \{[\s\S]*?rgba\(226, 232, 240, 0\.7\)/);
});
