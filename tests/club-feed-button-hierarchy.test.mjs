...
test("mobile Clubs cards keep one cross-platform header texture and a prominent active Deal", () => {
  ...
  assert.match(parity, /Android retains the shared direct-paint path[\s\S]*?html:is\([\s\S]*?\.is-android,[\s\S]*?\.android-rendering,[\s\S]*?\.is-samsung-browser,[\s\S]*?\.samsung-rendering[\s\S]*?\.home-venue-discovery-art:not\(\.has-custom-photo\) \{[\s\S]*?background-color: #353941 !important;[\s\S]*?rgba\(248, 250, 252, 0\.13\)[\s\S]*?rgba\(139, 92, 246, 0\.20\)[\s\S]*?#666b75 0%[\s\S]*?#353942 66%[\s\S]*?#4f545e 100%/);
  ...
  assert.doesNotMatch(parity, /--dancr-mobile-venue-header|@supports not \(-webkit-touch-callout: none\)|filter: brightness|opacity:/);
