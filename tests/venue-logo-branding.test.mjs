...
test("fictional Vegas venue travel uses one active MyDancr destination", () => {
  ...
  assert.match(liveApp, /function uberRideLinkMarkup[\s\S]*?const href = `\/rides\/\$\{encodeURIComponent\(venue.id\)\}\$\{query.size[\s\S]*?data-free-ride-link/);
  ...
  assert.match(uberButton, /const href = `\/rides\/\$\{encodeURIComponent\(venue.id\)\}[\s\S]*?<a[\s\S]*?href=\{href\}/);
});
...
test("venue scroll cards use one fixed logo stage while detail pages retain contain-fit identity", () => {
  ...
  assert.match(
    aesthetic,
    /\.home-venue-discovery-art \.home-venue-discovery-logo-shell \{[\s\S]*?top: 5%;[\s\S]*?right: 72px;[\s\S]*?bottom: auto;[\s\S]*?left: 18px;[\s\S]*?height: 50%;[\s\S]*?transform: none;[\s\S]*?border: 0;[\s\S]*?background: transparent;[\s\S]*?box-shadow: none;/,
  );
  assert.match(
    aesthetic,
  ...
  assert.match(aesthetic, /--venue-logo-shift-x[\s\S]*?--venue-logo-shift-y[\s\S]*?--venue-logo-scale/);
  assert.match(
    aesthetic,
    /@media \(max-width: 650px\) \{[\s\S]*?\.venue-detail-logo-shell \{[\s\S]*?left: 50% !important;/,
  );
