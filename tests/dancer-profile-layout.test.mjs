...
test("full dancer profiles use a compact identity and honest public activity header without a bio", () => {
  ...
  assert.match(liveApp, /class="profile-modal-avatar-column">[\s\S]*?id="modalProfileAvatar"[\s\S]*?class="modal-identity-stack">[\s\S]*?class="profile-modal-context">[\s\S]*?id="modalCity"/);
...
test("active full-profile Club Deals render a compact cashier-tap action and use one live-status color", () => {
  ...
  assert.match(
    liveApp,
    /profile-club-deal-tile\.is-active \.profile-club-deal-qr-button \{[\s\S]*?border-color: var\(--dancr-color-success-strong\) !important;[\s\S]*?var\(--dancr-color-success\) 64%,[\s\S]*?background-image: none !important;/,
  );
...
test("live dancer essentials stay compact above media and clear the mobile dock", () => {
  ...
  assert.match(
    liveApp,
    /Current Shift is a single destination row[\s\S]*?#profileBackdrop #profileModal \.profile-tonight-card > \.working-now-tile \{[\s\S]*?grid-template-columns: max-content minmax\(0, 1fr\) !important;[\s\S]*?gap: 6px !important;[\s\S]*?min-height: 0 !important;[\s\S]*?padding: 4px 8px !important;/,
  );
  assert.match(
    liveApp,
  ...
  assert.match(profilePage, /\.profile-media-section \{[^}]*margin-top: 12px;/);
  assert.match(
    profilePage,
...
test("Current Shift uses a quieter club row and secondary ride action", () => {
  assert.match(
    liveApp,
    /Current Shift is a single destination row[\s\S]*?#profileBackdrop #profileModal \.profile-tonight-card > \.working-now-tile \{[\s\S]*?gap: 6px !important;[\s\S]*?min-height: 0 !important;[\s\S]*?padding: 4px 8px !important;/,
  );
  assert.match(
    liveApp,
    /#profileBackdrop \.working-now-tile > \.profile-schedule-primary \{[\s\S]*?font-size: 10px !important;[\s\S]*?font-weight: 900 !important;[\s\S]*?letter-spacing: \.065em !important;/,
  );
  assert.match(
    liveApp,
  ...
  assert.match(
    liveApp,
    /#profileBackdrop \.working-now-tile \.profile-venue-destination\.is-live > \.venue-dot \{[\s\S]*?width: 18px !important;[\s\S]*?height: 18px !important;[\s\S]*?border: 0 !important;[\s\S]*?background: transparent !important;[\s\S]*?box-shadow: none !important;/,
  );
  assert.match(
    liveApp,
    /#profileBackdrop \.working-now-tile \.profile-venue-destination\.is-live > \.venue-dot svg \{[\s\S]*?width: 18px !important;[\s\S]*?height: 18px !important;[\s\S]*?background: transparent !important;[\s\S]*?filter: none !important;/,
  );
  assert.match(
    liveApp,
...
test("the in-profile TV tab is dancer-only, opens scroll cards, and does not alter global navigation", () => {
  assert.match(
    profilePage,
  ...
  assert.match(
    liveApp,
    /const params = new URLSearchParams\(\{[\s\S]*?city,[\s\S]*?dancer: profile\.id,[\s\S]*?limit: String\(MAX_DANCER_PROFILE_VIDEOS\)[\s\S]*?fetchJson\(`\/api\/public\/tv\?\$\{params\.toString\(\)\}`/,
  );
