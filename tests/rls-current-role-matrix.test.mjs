...
test('the current fixture uses all captured policies, view security and column grants',async()=>{
 assert.equal(tables.length,79);assert.equal(snapshot.policies.length,140);
 ...
 assert.equal((await db.query("select count(*)::int as n from pg_class c join pg_namespace n on n.oid=c.relnamespace where n.nspname='public' and c.relkind='r' and c.relrowsecurity")).rows[0].n,79);
