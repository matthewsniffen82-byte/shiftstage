-- Add an unused service-only submission transaction; preserve all existing data,
-- takedown/restoration helpers, appeal eligibility, policies and legal fields.
begin;
set local lock_timeout = '3s';
set local statement_timeout = '20s';

create function public.submit_dmca_counter_notice_safely(p_user_id uuid, p_case_id uuid, p_details jsonb)
...
begin
  ...
  if found then
    ...
    if not found then raise exception 'COUNTER_NOTICE_TRANSITION_UNCONFIRMED' using errcode='40001'; end if;
  end if;
...
$function$;
revoke all on function public.submit_dmca_counter_notice_safely(uuid,uuid,jsonb) from public,anon,authenticated;
grant execute on function public.submit_dmca_counter_notice_safely(uuid,uuid,jsonb) to service_role;
comment on function public.submit_dmca_counter_notice_safely(uuid,uuid,jsonb) is
  'Server-only uploader counter-notice and case transition. Same case and identical legal details replay the stored receipt without overwriting decisions; external delivery is separate.';
commit;
