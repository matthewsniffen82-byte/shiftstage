-- New qualifying Club Deal redemptions attributed to a dancer profile use a
-- monthly 30% / 40% / 50% dancer commission scale. Existing ledger entries
-- remain unchanged so settled and payable financial history stays auditable.

begin;

alter table public.deal_revenue_events
  alter column policy_version set default 'dancer-profile-monthly-30-40-50-v1';
...
declare
  ...
  v_revenue_id uuid;
  v_policy_version constant text := 'dancer-profile-monthly-30-40-50-v1';
begin
...
comment on function public.confirm_deal_redemption_from_nfc(text, uuid, uuid, jsonb) is
  'Atomically confirms cashier NFC redemption and snapshots the monthly 30%, 40%, or 50% dancer share for dancer-profile attribution.';
