-- Keep each cashier redemption's revenue total and recipient rows consistent.
-- This replaces one function only; existing records, rates, eligibility, locks
-- and execution privileges are preserved. No historical rows are recalculated.
-- Rollback: restore the pre-migration function definition after assessing the
-- original snapshot race. No data rollback or ledger deletion is required.
begin;
