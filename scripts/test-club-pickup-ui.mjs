...
const component=mode==='request'?React.createElement(Form,{venue:{id:'96000000-0000-4000-8000-000000000010',name:'Test Club',slug:'test-club',club_pickup_enabled:true}}):mode==='inbox'?React.createElement(Inbox):React.createElement(Chat,{requestId:'96000000-0000-4000-8000-000000000020'});
...
const css=['public/dancr-brand-tokens.v1.css','public/dancr-button-system.v1.css','public/dancr-aesthetic.v1.css','app/pickups/pickup.css'].map(file=>readFileSync(resolve(root,file),'utf8')).join('\n');
...
try {
  for(const [name,engine,device] of [['android',chromium,'Pixel 5'],['iphone',webkit,'iPhone 13']]) {
    ...
    try {
      ...
      let role='customer',consented=true,status='requested',failNextSend=true;const actions=[],messages=[{id:id(30),sequence:1,sender_type:'system',message_text:'Pickup requested from the venue.',created_at:'2026-09-14T19:00:00Z'}];
