...
assert.ok(html.includes(`/live-shell.js?v=${version}`));
const response = await fetch(`${base}/live-shell.js?v=${version}`);
assert.equal(response.status, 200);
const delivered = await response.text();
assert.equal(delivered, extractLiveShellAppScript(source));
const health = [];
